# ─────────────────────────────────────────────────────────────────────────────
# STANDALONE MODULE for the receipt-scanner repo (lives next to app.py).
# Registers GET /api/agent/c7-summary on the existing Flask app.
#
# To activate, add ONE line at the VERY BOTTOM of app.py (after all routes):
#     import c7_summary_route  # noqa: F401  (registers /api/agent/c7-summary)
#
# Auth is inherited from app.py's @app.before_request _require_auth hook,
# which already accepts ?token=<AGENT_API_TOKEN>.
# NOTE: deploy runs under gunicorn (app:app), where this import pattern is
# safe. If you ever run "python app.py" locally, the route still registers.
# ─────────────────────────────────────────────────────────────────────────────
...
# Commerce7 credentials: prefer the globals app.py already defines for the
# proxy; fall back to environment variables with the same names.
try:
    from app import C7_APP_ID, C7_APP_SECRET, C7_TENANT
except ImportError:
